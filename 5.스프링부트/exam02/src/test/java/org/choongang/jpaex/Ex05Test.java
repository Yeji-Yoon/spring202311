...
public class Ex05Test {
    ...
    void test3(){
        QBoardData boardData = QBoardData.boardData;
        List<BoardData> items = (List<BoardData>)repository.findAll(boardData.subject.contains("ahr"), Sort.by("createAt"));
        items.forEach(System.out::println);
    }
    ...
    void test5() {
        ...
        orBuilder.or(boardData.subject.contains("목")).or(boardData.content.contains("용"));//or은 반환값이 this//메서드 체이닝 의도

        andBuilder.and(orBuilder);
        //orbuilder는 andbuilder에 포함되게 하는것이 적합
        // (subject LIKE'%목%' OR content LIKE '%목%') AND seq IN(1,2,3);
        // 쿼리는 범위가 좁혀질수 있는 쿼리부터 오는게 좋음. or조건은 () 묶기
